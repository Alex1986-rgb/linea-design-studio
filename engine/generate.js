...
function drawFloor(room, sheet) {
  ...
  b += notesBlock(M - WT, ly + 34, [
    ...
    wetFloor ? 'Пол мокрой зоны ниже жилых на 20 мм; гидроизоляция с заходом на стены 200 мм.' : 'Укладка от базовой линии, указанной стрелкой; подрезка — в зоне, скрытой мебелью.',
...
const NODES = [
  {
    ...
    notes: [
      ...
      'Демпферная лента по периметру обязательна, шов у стены 10 мм закрывается плинтусом.',
  ...
  },
  {
    ...
    layers: [
      ...
      { t: 50, name: 'Стяжка ЦПС с уклоном к трапу/выпуску', fill: '#D6D0C2' },
    ...
    notes: [
      'Гидроизоляция заходит на стены на 200 мм, в зоне душа — на 1800 мм.',
      'Пол мокрой зоны ниже жилых на 20 мм; порог не устраивается, стык — на оси полотна.',
...
function drawFlatFloors(sheetNo) {
  return flatSheet(sheetNo, 'План напольных покрытий', 'Типы покрытий, направление укладки, отметки уровней', base => {
    ...
    // отметки уровня чистого пола: мокрые зоны на 20 мм ниже жилых (канон, floors)
    s += flatRooms.map(r => {
      const wet = r.type === 'bathroom' || r.type === 'wc';
      return levelPlan(base.fx(r.pos.x + r.w / 2) - 26, base.fy(r.pos.y + r.l / 2) + 14, wet ? -20 : 0, '');
    }).join('');
    return s;
  }, (x, y, w) => {
