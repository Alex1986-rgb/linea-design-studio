...
// а ряд для них — свой; подпись берётся из фактически выбранного коэффициента.
const NODE_SERIES = [[5, 9.4488], [10, 4.7244], [20, 2.3622]];
...
// Нумерованные примечания листа (ГОСТ 2.316 — надписи на чертежах; канон, раздел 2).
// Одна прозаическая строка «примечания: …» этого не заменяет: подрядчик читает пунктами.
// Наборы строк легенды по слоям — чтобы обозначения были одинаковы на всех листах.
function legendRowsFor(layer) {
  const g = CAD.furn, e = CAD.elec;
  const box = (c, fill) => (sx, sy) => `<rect x="${sx}" y="${sy - 9}" width="16" height="11" fill="${fill || 'none'}" stroke="${c}" stroke-width="0.9"/>`;
  const dash = c => (sx, sy) => `<line x1="${sx}" y1="${sy - 4}" x2="${sx + 16}" y2="${sy - 4}" stroke="${c}" stroke-width="1.1" stroke-dasharray="5 3"/>`;
  if (layer === 'plan') return [
    { sym: box(CAD.wallStroke, CAD.wallFill), text: 'существующая стена / перегородка в сечении' },
    { sym: box(g), text: 'мебель и оборудование, габарит в мм' },
    { sym: dash(g), text: 'зона открывания дверей и выдвижных элементов' },
    { sym: (sx, sy) => `<circle cx="${sx + 8}" cy="${sy - 3}" r="7" fill="none" stroke="#57514A" stroke-width="0.9"/><text x="${sx + 8}" y="${sy}" font-size="7.5" text-anchor="middle" fill="#2E2A26">n</text>`, text: 'номер помещения по экспликации' },
  ];
  if (layer === 'demolition') return [
    { sym: (sx, sy) => `<g><rect x="${sx}" y="${sy - 9}" width="16" height="11" fill="#FFF" stroke="#B0483A" stroke-width="0.8"/><line x1="${sx}" y1="${sy + 2}" x2="${sx + 16}" y2="${sy - 9}" stroke="#B0483A88" stroke-width="1"/></g>`, text: 'демонтаж отделки: покрытие пола, обои/краска, плинтусы' },
    { sym: (sx, sy) => `<g stroke="#B0483A" stroke-width="1.6"><line x1="${sx}" y1="${sy - 9}" x2="${sx + 14}" y2="${sy + 1}"/><line x1="${sx}" y1="${sy + 1}" x2="${sx + 14}" y2="${sy - 9}"/></g>`, text: 'демонтаж дверного блока: полотно, коробка, наличники' },
    { sym: box(CAD.wallStroke, CAD.wallFill), text: 'конструкции, которые не затрагиваются' },
  ];
  if (layer === 'electro') return [
    { sym: (sx, sy) => `<g stroke="${e}" stroke-width="1.2" fill="none"><path d="M ${sx + 3} ${sy - 3} a 5 5 0 0 1 10 0" fill="#FFF"/><line x1="${sx + 2}" y1="${sy - 3}" x2="${sx + 14}" y2="${sy - 3}"/></g>`, text: 'розетка 220 В; подпись — привязка от угла / высота от пола' },
    { sym: (sx, sy) => `<g stroke="${e}" stroke-width="1.2"><path d="M ${sx + 3} ${sy - 3} a 5 5 0 0 1 10 0" fill="${e}"/><line x1="${sx + 2}" y1="${sy - 3}" x2="${sx + 14}" y2="${sy - 3}"/></g>`, text: 'розетка IP44 (санузел, зона мойки) — через УЗО 30 мА' },
    { sym: (sx, sy) => `<g stroke="${e}" stroke-width="1.2"><circle cx="${sx + 6}" cy="${sy - 3}" r="3.4" fill="${e}"/><line x1="${sx + 6}" y1="${sy - 6}" x2="${sx + 13}" y2="${sy - 11}"/></g>`, text: 'выключатель, h=900 со стороны ручки двери' },
    { sym: box('#C9C9C9'), text: 'мебель — подложка для привязки, не входит в объём работ' },
  ];
  if (layer === 'smart') return [
    { sym: (sx, sy) => `<rect x="${sx}" y="${sy - 9}" width="13" height="12" rx="2" fill="#FFF" stroke="#3A7ABD" stroke-width="1"/><text x="${sx + 6.5}" y="${sy}" font-size="6.5" text-anchor="middle" fill="#3A7ABD">S</text>`, text: 'сценарный выключатель / панель управления' },
    { sym: (sx, sy) => `<circle cx="${sx + 8}" cy="${sy - 3}" r="6" fill="none" stroke="#3A7ABD" stroke-width="1"/>`, text: 'датчик (движение, протечка, климат)' },
    { sym: dash('#3A7ABD'), text: 'линия управления к модулю в щите' },
    { sym: box('#C9C9C9'), text: 'мебель — подложка для привязки' },
  ];
  return [];
}
...
function drawObmer(room, sheet) {
  ...
  const Wd = Math.max(760, w + M * 2 + 90), Hd = l + M * 2 + 196;
...
<text x="${lbx}" y="${lby + 20}" font-size="10" fill="#57514A">P=${per} м · h=${room.h}</text></g>`;
  ...
  const ny = M + l + 66;
  b += notesBlock(M - WT, ny, [
    'Все размеры в мм по внутренним поверхностям стен, без отделочного слоя.',
    'Размеры проверять по месту; допуск обмера ±5 мм в зонах встроенной мебели и санузлов.',
    'За отметку 0,000 принят уровень чистового пола.',
  ]);
  b += stamp(M - WT, ny + 56, w + 2 * WT + 60, `Обмерный план. ${room.name}`, sheet);
...
function drawFloor(room, sheet) {
  ...
  const Wd = Math.max(760, w + M * 2 + 60), Hd = l + M * 2 + 216;
  ...
  // отметка уровня чистого пола: в мокрых зонах на 20 мм ниже жилых (канон, floors)
  const wetFloor = room.type === 'bathroom' || room.type === 'wc';
  b += levelPlan(M + 8, M + 8, wetFloor ? -20 : 0, wetFloor ? 'чистый пол мокрой зоны' : 'чистый пол');
  b += notesBlock(M - WT, ly + 34, [
    'Стыки покрытий — на оси дверного полотна, без порожков.',
    wetFloor ? 'Пол на 15–20 мм ниже смежных помещений (СП 29.13330.2011, 4.8); гидроизоляция с заходом на стены не менее 200 мм.' : 'Укладка от базовой линии, указанной стрелкой; подрезка — в зоне, скрытой мебелью.',
    'Площади даны с запасом на подрезку: +15% «ёлка», +10% плитка.',
  ]);
  b += stamp(M - WT, ly + 92, w + 2 * WT + 40, `План пола. ${room.name}`, sheet);
...
function drawDemolition(room, sheet) {
  const M = 100, WT = 12, w = px(room.w), l = px(room.l);
  const Wd = Math.max(760, w + M * 2 + 60), Hd = l + M * 2 + 254;
  const wet = room.type === 'bathroom';
  let b = roomWalls(M, WT, room, sheet, CAD.paper);
...
<text x="${M - WT + 24}" y="${ly}">демонтаж отделки: ${wet ? 'плитка стен и пола, стяжка до плиты, старая гидроизоляция' : 'покрытие пола до стяжки, обои/краска, плинтусы, наличники'}</text></g>`;
  ...
  b += chainDimH(M, M + l + 30, room.w, wallOpenings(room, 'C'), true);
  b += chainDimV(M + w + 30, M, room.l, wallOpenings(room, 'B'), true);
  b += flatLegendBox(M - WT, ly + 30, 300, 'Условные обозначения', legendRowsFor('demolition'));
  b += notesBlock(M - WT + 320, ly + 48, [
    'Несущие конструкции не затрагиваются.',
    'Перед штроблением уточнить трассы скрытых коммуникаций.',
    'Мусор выносить в мешках; проёмы защитить от пыли.',
  ]);
  b += stamp(M - WT, ly + 140, w + 2 * WT + 40, `Демонтаж. ${room.name}`, sheet);
  return svgDoc(Wd + 20, Hd + 30, b);
}
...
// ================================================================
// БИБЛИОТЕКА УЗЛОВ (ГОСТ 21.501, табл. 1 — узлы М 1:5…1:20)
// Узел — единственное место, где встречаются стяжка, гидроизоляция, покрытие,
// плинтус и потолок. Именно на этих стыках бригада ошибается, поэтому пирог
// рисуется послойно, каждый слой — с выноской-полкой и толщиной.
// ================================================================
const NODES = [
  {
    key: 'uzel-B-pol-suhoy', mark: 'Б', ratio: 10, width: 900,
    title: 'Узел Б · Пирог пола жилой зоны',
    sub: 'применяется во всех жилых помещениях · отметка чистого пола 0,000',
    layers: [
      { t: 200, name: 'Плита перекрытия ж/б', fill: '#B9B2A4', hatch: true },
      { t: 20, name: 'Звукоизоляция (мат под стяжку)', fill: '#E4DFD2' },
      { t: 60, name: 'Стяжка ЦПС армированная, М150', fill: '#D6D0C2' },
      { t: 3, name: 'Подложка 2–3 мм', fill: '#EDE7DC' },
      { t: 15, name: 'Инженерная доска, укладка «ёлка»', fill: '#CBAE85' },
    ],
    notes: [
      'Стяжка выводится в ноль по всей квартире: перепад более 2 мм на 2 м — переделка.',
      'Зазор плавающей стяжки у стен, перегородок и трубопроводов — 25–30 мм на всю толщину, заполняется звукоизоляционным материалом (СП 29.13330.2011, 8.10).',
      'Зазор покрытия у стены 10 мм закрывается плинтусом; плинтус крепится к стене, не к покрытию.',
      'Тёплый пол в жилых зонах не предусмотрен; в стяжке — только трассы электрики в гофре.',
    ],
  },
  {
    key: 'uzel-V-pol-mokryy', mark: 'В', ratio: 10, width: 900, wet: true,
    title: 'Узел В · Пирог пола мокрой зоны с гидроизоляцией',
    sub: 'санузлы · отметка чистого пола −0,020 · гидроизоляция с заходом 200 мм на стены',
    layers: [
      { t: 200, name: 'Плита перекрытия ж/б', fill: '#B9B2A4', hatch: true },
      { t: 2, name: 'Гидроизоляция обмазочная, 2 слоя', fill: '#7FA8C8' },
      { t: 50, name: 'Стяжка ЦПС, уклон 1–2% к трапу в душевой (СП 30.13330, 17.9)', fill: '#D6D0C2' },
      { t: 6, name: 'Мат тёплого пола 150 Вт/м² + клей', fill: '#E5B9B6' },
      { t: 5, name: 'Клей плиточный, гребёнка 6 мм', fill: '#E8E3D6' },
      { t: 10, name: 'Керамогранит, шов 2 мм, затирка эпоксидная', fill: '#DED8CB' },
    ],
    notes: [
      'Гидроизоляция заходит на стены не менее 200 мм от уровня покрытия, в зоне прямых струй — на всю высоту замачивания (СП 29.13330.2011, 7.2).',
      'Слоёв не менее двух; под трапом и в радиусе 1 м от него — на два слоя больше, под трапом только оклеечная (СП 29.13330.2011, 7.3, 7.4).',
      'Пол мокрой зоны на 15–20 мм ниже смежных помещений либо отделён порогом высотой не более 20 мм (СП 29.13330.2011, 4.8).',
      'Датчик тёплого пола — в гофре между витками мата, доступ через терморегулятор.',
    ],
  },
  {
    key: 'uzel-G-styk-pokrytiy', mark: 'Г', ratio: 10, width: 900, joint: true,
    title: 'Узел Г · Стык покрытий на оси дверного полотна',
    sub: 'переход инженерная доска / керамогранит · без порожка',
    layers: [
      { t: 200, name: 'Плита перекрытия ж/б', fill: '#B9B2A4', hatch: true },
      { t: 60, name: 'Стяжка: слева 60, справа 50 + гидроизоляция', fill: '#D6D0C2' },
      { t: 18, name: 'Слева — доска 15 по подложке 3', fill: '#CBAE85' },
    ],
    notes: [
      'Стык — строго по оси дверного полотна в закрытом положении.',
      'Соединение скрытым профилем в уровень покрытий, перепад покрытий 20 мм гасится подложкой.',
      'Шов заполняется эластичным герметиком в цвет темного покрытия.',
    ],
  },
  {
    key: 'uzel-D-plintus-shov',  mark: 'Д', ratio: 10, width: 700, shadow: true,
    title: 'Узел Д · Примыкание стены к полу: плинтус и теневой шов',
    sub: 'два исполнения: плинтус МДФ 80 (жилые) и теневой шов 10×20 (скрытое)',
    layers: [
      { t: 12.5, name: 'ГКЛ 12,5 по профилю (стена)', fill: '#F6F3EC' },
      { t: 80, name: 'Плинтус МДФ 80, эмаль в цвет стен', fill: '#E4DFD2' },
      { t: 15, name: 'Покрытие пола, зазор у стены 10 мм', fill: '#CBAE85' },
    ],
    notes: [
      'Плинтус крепится к стене, не к покрытию: покрытие должно двигаться.',
      'Теневой шов 10×20 — алюминиевый профиль, устанавливается до чистовой отделки.',
      'В мокрых зонах плинтус не устраивается: керамогранит выводится на стену.',
    ],
  },
];
...
function drawNode(sheet) {
  const M = 110;
  const q = px;   // модельные единицы: масштаб узла выбирается из ряда 1:5 / 1:10 / 1:20
  const X = M, Y = M;
  let b = '';
  // перекрытие
  ...
  {
    const inkN = inkMap();
    const l1 = leader(inkN, X + q(220), y1 + 6, 'ГКЛ 12,5 по ПП 60×27', { size: 8.6, arm: 34, shelf: 44 });
    if (l1) b += l1;
    const l2 = leader(inkN, X + q(480), y2 + 4, 'профиль LED, полка 100', { size: 8.6, arm: 30, shelf: 44 });
    if (l2) b += l2;
  }
...
function drawElectro(room, sheet) {
  const M = 90, WT = 12, w = px(room.w), l = px(room.l);
  const Wd = Math.max(760, w + M * 2 + 60), Hd = l + M * 2 + 228;
  ...
  b += `<g data-el="legend"></g><text x="${M - WT}" y="${ly - 16}" font-size="10" font-weight="700" fill="#2E2A26">Условные обозначения</text>`;
  ...
  b += stamp(M - WT, l + M * 2 + 172, w + 2 * WT + 40, `Электрика. ${room.name}`, sheet);
  return svgDoc(Wd + 20, Hd + 10, b);
}
...
function drawSmartHome(room, sheet) {
  ...
  const Wd = Math.max(860, w + M * 2 + SPEC_W + 40), Hd = l + M * 2 + 168;
  ...
  b += `<g data-el="legend"></g><text x="${M - WT}" y="${lyS - 16}" font-size="10" font-weight="700" fill="#2E2A26">Условные обозначения</text>`;
  ...
  b += stamp(M - WT, l + M * 2 + 112, w + 2 * WT + 40, `Умный дом. ${room.name}`, sheet);
  return svgDoc(Wd + 20, Hd + 10, b);
}
...
function leader(ink, tx, ty, text, opt) {
  ...
  for (const arm of arms) for (const [dx, dy] of LEAD_DIRS) {
    ...
    // Проверяем только место под ТЕКСТ: точка-указка по определению стоит на предмете,
    // а наклонная и полка — тонкие линии, их пересечение с графикой допустимо (ГОСТ 2.316).
    if (!ink.free(txx - 2, by - th + 2, tw + 4, th)) continue;
    ink.add(txx - 2, by - th + 2, tw + 4, th);
...
function drawFlatObmer(sheetNo) {
  return flatSheet(sheetNo, 'Обмерный план квартиры', `${esc((brief.object && brief.object.address) || '')} · ${totalArea} м² · все размеры в мм`, base => {
    ...
    for (const ax of ['X', 'Y']) {
      const sr = sectionRooms(ax);
      const nm = ax === 'X' ? '1' : '2';
      if (ax === 'X') {
        const y = base.fy(sr.mid), xa = base.fx(FLAT.x0) - px(EXT) - 34, xb = base.fx(FLAT.x1) + px(EXT) + 78;
        s += `<g stroke="#1C1C1C" stroke-width="1.6" fill="none"><line x1="${xa}" y1="${y}" x2="${xa + 26}" y2="${y}"/><line x1="${xb - 26}" y1="${y}" x2="${xb}" y2="${y}"/>`
          + `<path d="M ${xa + 26} ${y - 5} L ${xa + 36} ${y} L ${xa + 26} ${y + 5}" stroke-width="1.1"/><path d="M ${xb - 26} ${y - 5} L ${xb - 36} ${y} L ${xb - 26} ${y + 5}" stroke-width="1.1"/></g>`
          + `<text x="${xa}" y="${y - 9}" font-size="10.5" font-weight="700" fill="#1C1C1C">${nm}</text>`
          + `<text x="${xb - 8}" y="${y - 9}" font-size="10.5" font-weight="700" fill="#1C1C1C">${nm}</text>`;
      } else {
...
function drawFlatFurniture(sheetNo) {
  return flatSheet(sheetNo, 'Планировочное решение', `Расстановка мебели · шторы · выноски · референсы решений`, base => {
    ...
    const ink = inkMap();
    if (base.col) ink.add(base.col.x, base.col.y, base.col.w, base.col.h);   // правая колонка занята
...
function drawFlatFloors(sheetNo) {
  return flatSheet(sheetNo, 'План напольных покрытий', 'Типы покрытий, направление укладки, отметки уровней', base => {
    ...
    // Отметки уровня чистого пола и номер типа пола в равностороннем треугольнике
    // 8–10 мм (ГОСТ 21.501-2018, 5.5.1) — раньше тип пола на листе не маркировался.
    s += flatRooms.map(r => {
...
function drawFlatAC(sheetNo) {
  ...
  return flatSheet(sheetNo, 'Кондиционеры', 'Схема кондиционирования · внутренние блоки, трассы фреона, конденсат', base => {
    ...
    // уклон дренажной трассы обязателен на листе (канон, чек-лист climate)
    s += `<text x="${base.fx(FLAT.x0) + 12}" y="${base.fy(FLAT.y1) + px(EXT) + 18}" font-size="8.6" fill="#2E6FA8">Дренаж: уклон i ≥ 0,01 (10 мм/пог.м) в сторону стояка; на пересечении с коробом — в теле короба.</text>`;
    return s + flatRoomMarks(base.fx, base.fy, false);
  }, (x, y, w) => flatLegendBox(x, y, w, 'Условные обозначения', [
...
function drawFlatPlumbing(sheetNo) {
  ...
  return flatSheet(sheetNo, 'Сантехника с привязками', 'Расположение сантехнических приборов · привязки от стен в мм', base => {
    ...
    if (base.col) ink.add(base.col.x, base.col.y, base.col.w, base.col.h);
...
// ошибается чаще всего, а до сих пор в альбоме разрезов не было.
// ================================================================
function sectionRooms(axis) {
  // Секущую плоскость ведём через середину самого большого помещения: по середине
  // габарита она проходила по свободной зоне, и разрез выходил без мебели вовсе.
  const big = flatRooms.slice().sort((a, b) => b.w * b.l - a.w * a.l)[0] || flatRooms[0];
  const mid = axis === 'X' ? big.pos.y + big.l / 2 : big.pos.x + big.w / 2;
  const hit = flatRooms.filter(r => axis === 'X'
    ? (r.pos.y <= mid && r.pos.y + r.l >= mid)
    : (r.pos.x <= mid && r.pos.x + r.w >= mid));
  const list = (hit.length ? hit : flatRooms).slice().sort((a, b) =>
    axis === 'X' ? a.pos.x - b.pos.x : a.pos.y - b.pos.y);
  return { mid, list };
}

function drawFlatSection(axis, sheetNo) {
  const { mid, list } = sectionRooms(axis);
  const M = 110, WT = px(EXT_MM), INT = px(INT_MM);
  const along = r => axis === 'X' ? r.w : r.l;         // длина помещения по линии разреза
  const start = r => axis === 'X' ? r.pos.x : r.pos.y;
  const x0 = start(list[0]), x1 = start(list[list.length - 1]) + along(list[list.length - 1]);
  ...
  for (let i = 0; i < (x1 - x0) + 2 * EXT_MM; i += 300) b += `<line x1="${M + px(i)}" y1="${floorY + SLAB}" x2="${M + px(i) + px(150)}" y2="${floorY}" stroke="#8A8478" stroke-width="0.6"/>`;
  b += `<rect x="${M}" y="${M - SLAB}" width="${fx(x1) - M + WT}" height="${SLAB}" fill="#B9B2A4" stroke="${CAD.wallStroke}" stroke-width="1.2"/>`;

  // наружные стены в сечении
  for (const wx of [M, fx(x1)]) b += `<rect x="${wx}" y="${M}" width="${WT}" height="${px(H)}" fill="url(#secw)" stroke="${CAD.wallStroke}" stroke-width="1.3"/>`;
  b = `<defs><pattern id="secw" width="7" height="7" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="7" height="7" fill="${CAD.wallFill}"/><line x1="0" y1="0" x2="0" y2="7" stroke="${CAD.hatch}" stroke-width="2"/></pattern></defs>` + b;

  list.forEach((r, i) => {
    const rx = fx(start(r)), rw = px(along(r));
    const lv = ceilingLevelsFor(r);
    const wet = r.type === 'bathroom' || r.type === 'wc';
    const fy0 = floorY - (wet ? px(20) : 0);            // мокрая зона ниже жилых на 20 мм
    // пирог пола помещения
    b += `<rect x="${rx}" y="${fy0 - PIE}" width="${rw}" height="${PIE}" fill="${wet ? '#DED8CB' : '#CBAE85'}" stroke="${CAD.wallStroke}" stroke-width="0.7"/>`;
    // перегородка справа
    if (i < list.length - 1) b += `<rect x="${rx + rw}" y="${M}" width="${INT}" height="${px(r.h)}" fill="${CAD.wallFill}" stroke="${CAD.wallStroke}" stroke-width="1.1"/>`;
    // потолок: базовый уровень и короб
    const ceilY = floorY - px(r.h - CEIL_DROP1);
    b += `<rect x="${rx}" y="${ceilY}" width="${rw}" height="${px(12.5)}" fill="#F6F3EC" stroke="${CAD.wallStroke}" stroke-width="0.9"/>`;
    if (!lv.oneLevel) {
      const boxW = px(lv.box), boxH = px(120);
      b += `<rect x="${rx}" y="${ceilY}" width="${boxW}" height="${boxH}" fill="#E9E4D8" stroke="${CAD.wallStroke}" stroke-width="0.9"/>`;
      b += `<rect x="${rx + rw - boxW}" y="${ceilY}" width="${boxW}" height="${boxH}" fill="#E9E4D8" stroke="${CAD.wallStroke}" stroke-width="0.9"/>`;
    } else {
      b += `<text x="${rx + rw / 2}" y="${ceilY - 5}" font-size="8" fill="#57514A" text-anchor="middle">один уровень, люк ревизии</text>`;
    }
    // силуэты мебели, попадающей в плоскость разреза
    for (const f of furnitureFor(r)) {
      const cut = axis === 'X'
        ? (r.pos.y + f.y <= mid && r.pos.y + f.y + f.h >= mid)
        : (r.pos.x + f.x <= mid && r.pos.x + f.x + f.w >= mid);
      if (!cut) continue;
      const spec = FURN_H[f.key];
      if (!spec) continue;
      const fw = axis === 'X' ? px(f.w) : px(f.h);
      const fxx = rx + (axis === 'X' ? px(f.x) : px(f.y));
      const fh = px(spec.head && f.head ? spec.head : spec.h);
      b += `<rect x="${fxx}" y="${fy0 - PIE - fh}" width="${fw}" height="${fh}" fill="#2E9E4F10" stroke="${CAD.furn}" stroke-width="1"/>`;
      const lead = leader(ink, fxx + fw / 2, fy0 - PIE - fh / 2, `${(FURN_H[f.key] || {}).name || f.name} h=${spec.head && f.head ? spec.head : spec.h}`, { size: 8, arms: [26, 56, 96], shelf: 40 });
      if (lead) b += lead;
    }
  ...
  b += `<text x="${M}" y="${M - SLAB - 22}" font-size="11" fill="#7A756D">Секущая плоскость показана на обмерном плане · отметки от уровня чистого пола 0,000 · высоты мебели — из спецификации</text>`;
  b += notesBlock(M, floorY + SLAB + 104, [
    'Пирог пола — узлы Б (жилые) и В (мокрые зоны), примыкание к стене — узел Д.',
    `Низ первого уровня потолка ${mark(BASE_H)}, низ короба ${mark(BASE_H - 120)}; закладные под подвесные светильники — до зашивки ГКЛ.`,
    'Высоты мебели проверить по спецификации после контрольного замера на объекте.',
  ], 110);
  b += stamp(M, floorY + SLAB + 168, 620, name, sheetNo, '1:50');
